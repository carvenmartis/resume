...
function EditorSidebar({ previewOpen, onTogglePreview }: { previewOpen: boolean; onTogglePreview: () => void }) {
  ...
  return (
    ...
    >
      <div className={`flex-1 flex flex-col gap-6 p-6 overflow-y-auto ${previewOpen ? '' : 'max-w-lg mx-auto w-full'}`}>
        <div className="flex items-center justify-between">
          ...
          >
            <i className={`bx ${previewOpen ? 'bx-layout' : 'bx-expand-alt'} text-[1.2rem]`} />
          </button>
        </div>
      ...
      <div className={`p-6 border-t border-gray-200 dark:border-gray-700 ${previewOpen ? '' : 'max-w-lg mx-auto w-full'}`}>
        <PrintButton />
      </div>
    </motion.aside>
  )
